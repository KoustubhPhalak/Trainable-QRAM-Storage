# Code to run digit classification sims with QRAM (for 3 digits)
...
label3 = int(input("Enter third label:"))


indice = np.where((target == label1) | (target == label2) | (target == label3))
...
for epoch in range(qcls_start_epochs, qcls_epochs):
    ...
    curr_log += f"train acc:{train_acc/(len(train_loader.dataset)):.4f}\t test acc:{test_acc/(len(test_loader.dataset)):.4f}"
