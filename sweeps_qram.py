...
provider = IBMQ.enable_account('8fa44a709bb20f84997d142828fbc9b816343eb04a7b5e59e7b20d2c01dcb0748058d0ac92d27a1104131d1fbf22df4953d71cfc2aa0ba6b302261edcfa774b1')
...
# data = np.reshape(resize(images, (1797,4,4)), (1797, 16))
...
# first_zero = (labels==0).nonzero().squeeze()

# first_one = (labels==1).nonzero().squeeze()
...
def qram(inputs, params, weights):
    ...
    return qml.probs(wires=[0,1,2,n_qubits-3, n_qubits-2, n_qubits-1])
...
for epoch in range(qram_start_epoch, qram_epochs):
    ...
    print_and_save(curr_log, f"{log_dir}/qram_{classes_string}_{num_images}_{type(opt_qrams[cls]).__name__}_{lr}_{embedding}_log.txt")
    ...
    if (epoch+1) % save_step == 0:
        ...
        torch.save(save_dict, f"{models_dir}/qrams-{classes_string}-{num_images}-{type(opt_qrams[cls]).__name__}-{lr}-{embedding}-{epoch+1}.pth")
...
for epoch in range(qcls_start_epochs, qcls_epochs):
    ...
    print_and_save(curr_log, f"{log_dir}/model-{classes_string}_{num_images}_{type(opt_qrams[cls]).__name__}_{lr}_{embedding}_log.txt")

    if (epoch+1) % save_step == 0:
        torch.save({'model_state_dict': model.state_dict()}, f"{models_dir}/model-{classes_string}-{num_images}-{type(opt_qrams[cls]).__name__}-{lr}-{embedding}-{epoch+1}.pth")

# print(f"FIRST ZERO LABEL INDEX: {first_zero[0:5]}, ADDRESS: {adds[first_zero[0:5]]}")  
# print(f"FIRST ONE LABEL INDEX: {first_one[0:5]}, ADDRESS: {adds[first_one[0:5]]}")        